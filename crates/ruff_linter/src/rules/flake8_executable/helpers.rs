...
/// Returns `true` if the current process is running under WSL (Windows Subsystem for Linux).
///
/// Containers running on a WSL2 host are **not** considered WSL, since they provide
/// a standard Linux filesystem with proper executable-bit semantics. The `is-wsl`
/// crate gets this wrong because its `is-docker` dependency fails to detect
/// non-Docker container runtimes (e.g., Azure's hosted containers use
/// `/containers/` cgroup paths and no `/.dockerenv`).
///
/// See: <https://github.com/astral-sh/ruff/issues/10084>
